...
public class Options
{
    ...
    // GameNative fork: when false, no background SST compaction is scheduled. for read-only /
    // transient opens (e.g. save-sync's synthesize+rewrite reads), compaction is pure overhead AND
    // a correctness hazard: it retires/unmaps L0 SSTs that a concurrent iterator is still reading,
    // intermittently dropping records ("missing key" after open). default true preserves behavior.
    ...
    public Options compactionEnabled(boolean compactionEnabled)
    {
