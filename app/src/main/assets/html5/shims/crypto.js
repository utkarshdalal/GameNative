// pure-JS require("crypto") exposing only createDecipheriv('aes-256-ctr'), for games that decrypt their own
// bundled data (e.g. OMORI). CTR: XOR data with AES(counter), counter = IV incremented big-endian per block.
//
// NOT safe for real crypto: no constant-time ops or side-channel resistance. fine only because the key and IV
// ship publicly inside the game.
(function () {
    'use strict';
    ...
    function wrapBytesMaybe(bytes) {
        // a Buffer lets callers chain .toString('utf8'); most node code tolerates a bare Uint8Array otherwise.
    ...
    function createAesCtrDecipher(key, iv) {
        ...
        return {
            ...
            final: function () {
                // CTR has no padding, so final is always empty.
