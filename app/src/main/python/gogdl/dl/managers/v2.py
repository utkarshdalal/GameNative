...
class V2Manager:
    ...
    def download(self):
        ...
        try:
            ...
            # Only add DLC product IDs if DLCs should be downloaded
            dlcs_should_be_downloaded = getattr(self.arguments, 'dlcs', False)
            if dlcs_should_be_downloaded:
                # Extract product IDs from depot files
                for depot in depots:
                    if 'productId' in depot and depot['productId'] != self.game_id:
                        product_ids.add(depot['productId'])
                self.logger.info(f"DLCs enabled - will download DLC content")
            else:
                self.logger.info(f"DLCs disabled - skipping DLC content")
