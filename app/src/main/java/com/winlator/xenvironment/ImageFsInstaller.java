...
public abstract class ImageFsInstaller {
    ...
    public static void installWineFromAssets(final Context context, AssetManager assetManager) {
        // Install all wine variants (GLIBC, BIONIC, and any future variants)
        int[] wineArrayIds = {R.array.glibc_wine_entries, R.array.bionic_wine_entries};
        File rootDir = ImageFs.find(context).getRootDir();

        for (int arrayId : wineArrayIds) {
            String[] versions = context.getResources().getStringArray(arrayId);
    ...
    public static Future<Boolean> installIfNeededFuture(final Context context, AssetManager assetManager, Container container, Callback<Integer> onProgress) {
        ...
        if (!imageFs.isValid() || imageFs.getVersion() < LATEST_VERSION || !imageFs.getVariant().equals(container.getContainerVariant())) {
            ...
            // Check if wine assets need to be installed (might be missing if app was updated)
            File optDir = new File(imageFs.getRootDir(), "opt");
            boolean needsWineInstall = false;

            if (!optDir.exists() || optDir.listFiles() == null || optDir.listFiles().length == 0) {
                Log.d("ImageFsInstaller", "Opt dir doesn't exist or is empty");
                needsWineInstall = true;
            } else {
                Log.d("ImageFsInstaller", "Checking for wine versions in opt dir");
                // Check if all expected wine versions exist
                int[] wineArrayIds = {R.array.glibc_wine_entries, R.array.bionic_wine_entries};
                for (int arrayId : wineArrayIds) {
                    String[] versions = context.getResources().getStringArray(arrayId);
                    Log.d("ImageFsInstaller", "Checking " + versions.length + " wine versions");
                    for (String version : versions) {
                        File wineDir = new File(optDir, version);
                        Log.d("ImageFsInstaller", "Wine version: " + version + ", exists: " + wineDir.exists());
                        if (!wineDir.exists()) {
                            Log.d("ImageFsInstaller", "Wine version missing: " + version);
                            needsWineInstall = true;
                            break;
                        }
                    }
            ...
            if (needsWineInstall) {
                Log.d("ImageFsInstaller", "Wine assets missing, installing from assets");
                return Executors.newSingleThreadExecutor().submit(() -> {
                    installWineFromAssets(context, assetManager);
                    return true;
                });
