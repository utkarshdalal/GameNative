...
public class FSREffect extends Effect {
    ...
    protected void onUse(ShaderMaterial material, GLRenderer renderer) {
        // Safe limit between 0.0 (off) and 2.0 (high sharpness)
        material.setUniformFloat("sharpness", Math.max(0.0f, Math.min(sharpness, 2.0f)));
    }

    private static class FSRMaterial extends ScreenMaterial {
        public FSRMaterial() {
        ...
        protected String getFragmentShader() {
            return "precision highp float;\n" +
                ...
                "void main() {\n" +
                "    vec2 inputSizeInPixels = resolution;\n" +
                ...
                "\n" +
                "    // Inline RCAS sharpening pass using the EASU output and original texture fetches\n" +
                ...
                "    \n" +
                "    float minRGB = min(min(min(min(cB.g, cD.g), cF.g), cH.g), pix.g);\n" +
                "    float maxRGB = max(max(max(max(cB.g, cD.g), cF.g), cH.g), pix.g);\n" +
                "    float limit = clamp(min(minRGB, 1.0 - maxRGB) / max(maxRGB, 1e-5), 0.0, 1.0);\n" +
                "    float w = sharpness * limit;\n" +
                "    vec3 rcas = (pix + w * (cB + cD + cF + cH)) / (1.0 + 4.0 * w);\n" +
                "    \n" +
