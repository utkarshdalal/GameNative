...
public class TouchpadView extends View implements View.OnCapturedPointerListener {
    ...
    public boolean onCapturedPointer(View view, MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_MOVE || action == MotionEvent.ACTION_HOVER_MOVE) {
            ...
            if (idx != 0 || idy != 0) {
                // Prevent X11 short overflow by clamping the candidate coordinates to screen bounds
                int candX = this.xServer.pointer.getX() + idx;
                int candY = this.xServer.pointer.getY() + idy;
                int clampedX = Mathf.clamp(candX, 0, this.xServer.screenInfo.width - 1);
                int clampedY = Mathf.clamp(candY, 0, this.xServer.screenInfo.height - 1);
        ...
        if (action == MotionEvent.ACTION_BUTTON_PRESS || action == MotionEvent.ACTION_BUTTON_RELEASE || action == MotionEvent.ACTION_SCROLL) {
