...
object SteamAchievementCodec {
    ...
    // close-time counterpart to seedEarnedState. launch seeding clears resets, but it only runs when the
    // game launched logged in, so a game played offline reaches the close/reconnect push with the stale
    // flag still on disk. a local unlock NEWER than the discarded time is a genuine re-earn -- keep it.
    @JvmStatic
    fun dropResetUnlocks(
        diskUnlocked: Set<String>,
        localUnlockTimes: Map<String, Long>,
        resets: Map<String, Long>,
    ): Set<String> = diskUnlocked.filterNotTo(mutableSetOf()) { name ->
        val resetAt = resets[name] ?: return@filterNotTo false
        (localUnlockTimes[name] ?: 0L) <= resetAt
    }
}
