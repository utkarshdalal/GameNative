...
// pins the SteamAutoCloud SHA cross-check on both the cache-absent and cache-present paths.
...
class SteamAutoCloudConflictDetectionTest {
    ...
    fun setUp() {
        ...
        // a single UFS pattern keeps the test deterministic -- only one prefixPath shape to reason
        // about when forging cache rows that drift vs. the live scan.
        val patterns = listOf(
            SaveFilePattern(
    ...
    private fun makeCloudFile(name: String, sha: ByteArray, sizeBytes: Int, prefixIndex: Int = 0): AppFileInfo {
        ...
        // prefix lookup is gated on the proto presence bit; mocks must set it.
    ...
    // seeds the cloud-side AppFileChangeList so the prefix maps to the SaveGames dir and the
    // file's absolute path is byte-identical to the local one. cloud CN advances past local CN
    // so sync takes the cloud-changed path.
    ...
    // cache absent (no FCL row, no CN row) + local SHA matches remote SHA -> silent rehydration.
    @Test
    ...
    // cache absent + local SHA diverges from remote -> conflict.
    @Test
    ...
    // cache present but its cloudPath encoding drifted + local SHA matches remote -> silent
    // rehydration. getFilesDiff reads the drift as local changes, so without the SHA cross-check
    // on this path it fires a conflict.
    @Test
    fun cachePresent_localMatchesRemoteSha_silentRehydrates() {
        val (content, sha) = writeSave("save.sav", "save content".toByteArray())

        // the cached row's cloudPath differs from the live scan's pattern.uploadPath; SHA stays
        // identical, only the path key drifts -- what an app update that changes prefix encoding
        // leaves behind.
        runBlocking {
            ...
            db.appFileChangeListsDao().insert(
                ...
                listOf(
                    UserFileInfo(
                        ...
                        // remote, so sync must short-circuit to UpToDate.
        ...
        // cloud CN advances past local CN (4 -> 5)
        stubCloudWithFile(cloudChangeNumber = 5, filename = "save.sav", sha = sha, sizeBytes = content.size)

        val result = runSync()
        assertNotNull("Result should not be null", result)
        assertEquals(
            "Cache present + path drift + local SHA matches remote → silent rehydration",
            SyncResult.UpToDate,
            result!!.syncResult,
        )
    }

    // cache present + local SHA diverges from remote -> conflict. the cross-check must not
    ...
    fun cachePresent_localDivergesFromRemoteSha_firesConflict() {
        ...
        // the cached row records a DIFFERENT sha than disk (the user modified saves offline). the
        // path is fine -- drift is only sha-side, so getFilesDiff fires on modifiedFiles.
        ...
        // cloud advertises yet a THIRD sha (another device wrote a new save) -> real divergence.
        // cloud CN > local CN.
