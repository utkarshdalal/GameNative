...
// the cache builder for the reconcile after a failed upload batch, keyed by filename so it needs
// none of the abs-path machinery the real call site uses. the two properties that matter:
...
class SteamAutoCloudUploadFailReconcileTest {
    ...
    fun localOnlyFile_excludedFromCache() {
        ...
        val cache = SteamAutoCloud.buildUploadFailCacheEntries(local, remote, keyOf)
    ...
    fun mixedSet_onlyInCloudKept_failedPutsRewritten() {
        ...
        val cache = SteamAutoCloud.buildUploadFailCacheEntries(local, remote, keyOf).associateBy { it.filename }
    ...
    fun emptyLocal_yieldsEmptyCache() {
        assertTrue(SteamAutoCloud.buildUploadFailCacheEntries(emptyList(), mapOf("x" to byteArrayOf(1)), keyOf).isEmpty())
    }
}
