...
class PackageJsonProbeTest {
    ...
    @Test fun parse_mainNotString_returnsNullMain() {
        val text = """{"main":42}"""
        val p = PackageJsonProbe.parse(text)
        // any primitive content is accepted, so a numeric main comes back as "42" -- signature
        // consumers downstream filter by .endsWith(".html") and reject it there.
    ...
    @Test fun parse_extractsDescription() {
        // Tyrano-on-Electron package.json shape (Welcome to Maison Chichigami).
        val text =
