...
// regular base shims + pack shims resolve by convention (no explicit map entry), while irregular
// ids (filename != id) keep explicit entries.
class ShimBundlesConventionTest {

    @Test
    fun regularBaseShim_derivesRootPath() {
        // no explicit entry -- must resolve via deriveShimPath to the identical path.
