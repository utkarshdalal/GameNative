...
class SteamworksJsBridgeGreenworksTest {
    ...
    // stubs configFile + load + save in their fully-resolved 3-arg/2-arg forms: mockk records
    // calls AT the resolved overload after Kotlin's $default trampoline runs, so verifying the
    // bridge's `save(slug, container)` requires matching `save(slug, any(), any())`.
