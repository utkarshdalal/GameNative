...
// SOURCE-level (grep) coverage of the require-dispatcher.js asset -- locks the register.pattern
// API contract so a later JS edit can't silently drop it. RequireDispatcherExecTest covers the
// runtime behavior.
class RequireDispatcherPatternTest {
    ...
    private fun source(): String {
        // gradle runs :app tests with cwd=app/ OR project root.
    ...
    @Test fun dispatch_order_exact_before_pattern() {
        // exact-match lookup (Object.prototype.hasOwnProperty.call) must appear before the
        // patternDispatchers scan.
        val src = source()
    ...
    @Test fun bare_module_miss_returns_undefined() {
        ...
        // expecting undefined-on-miss; throwing on every miss broke boot at require('buffer').
