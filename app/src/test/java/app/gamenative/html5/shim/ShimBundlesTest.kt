...
class ShimBundlesTest {
    ...
    // TOUCH_ID is the only touch bundle; the mode-specific touch shims are gone.
    ...
    // a clean unknown id derives a conventional path (the shim either exists there or 404s, same
    // as a typo'd asset filename). null is reserved for ids that can't be a safe filename
    // (traversal/dots/slashes/empty) -- see the deriveShimPath guard.
