...
// cases are exactly what a refactor breaks.
class ShimUrlSanitizeExecTest {
    private lateinit var js: ShimJsRuntime

    @Before
    fun setUp() {
        // installXhr records the final (post-sanitize) url in lastUrl, so san(u) returns whatever
        // the shim forwarded.
    ...
    fun normalizes_backslashes_before_detecting_scheme() {
        // file:\\data\x is canonicalized to / FIRST so indexOf('file://') matches; detecting
        // before normalizing would leak an unrewritten file:\\ url to chrome.
        assertEquals(
            "/data/x.ogg",
            js.evalString(
