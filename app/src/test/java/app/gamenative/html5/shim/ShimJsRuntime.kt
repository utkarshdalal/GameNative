...
// minimal Rhino harness that EXECUTES shims so their behavior is locked by real execution, not
// source-string assertions. the base runtime provides only window/self/console; the few shims
// that need more opt into the stubs below. DOM- and Kotlin-bridge-coupled behavior is validated
// on device, not here.
//
...
class ShimJsRuntime : AutoCloseable {
    ...
    // opt-in stubs -- NOT installed by default so the pure-shim path stays minimal.
