...
class IndexHtmlRewriterTest {
    ...
    fun termina_rmmv_fixture_injects_shim_before_first_game_script() {
        ...
        // canvasFixStyle shifts absolute indices, so compare against a substring unique to the
        // first game script.
    ...
    fun source_input_stream_is_not_mutated_on_disk() {
        // inject takes an InputStream; the fixture must be untouched afterwards.
    ...
    fun locale_string_is_json_escaped() {
        // hostile locale -- JSONObject.quote must escape the quote so the defineProperty literal
        // doesn't break out.
        val source = "<html><body><script src=\"g.js\"></script></body></html>"
        val rewritten = rewriteWithLocale(source, emptyList(), "en\"XSS")
        // escaped form must be present, not the raw en"XSS break-out.
    ...
    fun locale_script_sets_process_mainModule_filename() {
        // RMMZ's StorageManager.fileDirectoryPath reads process.mainModule.filename once fs.js's
        // isLocalMode override routes saves through the native path. typeof process stays
        // 'function' (YEP guard preserved) but mainModule must exist or boot crashes.
        val source = "<html><body><script src=\"g.js\"></script></body></html>"
