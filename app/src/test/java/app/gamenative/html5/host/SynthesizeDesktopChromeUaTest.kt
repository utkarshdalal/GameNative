...
// the spoof keeps the real Chromium milestone so it tracks WebView updates automatically.
class SynthesizeDesktopChromeUaTest {
    ...
    @Test fun output_shape_matches_canonical_desktop_chrome_ua() {
        // server-side UA parsers are sensitive to segment ordering, so the output must match the
        // canonical desktop Chrome format exactly.
        val out = synthesizeDesktopChromeUa(mobileUa)
        assertEquals(
