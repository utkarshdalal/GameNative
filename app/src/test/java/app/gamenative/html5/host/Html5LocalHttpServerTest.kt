...
// these are companion functions precisely so they're testable without constructing the server
// (its ctor binds 127.0.0.1).
class Html5LocalHttpServerTest {

    private fun reader(text: String) = java.io.BufferedReader(java.io.StringReader(text))

    @Test
    fun readLineBounded_stripsCrlfAndStopsAtEachLine() {
        val r = reader("GET /x HTTP/1.1\r\nHost: a.localhost:1\r\n\r\n")
        assertEquals("GET /x HTTP/1.1", Html5LocalHttpServer.readLineBounded(r, 8192))
        assertEquals("Host: a.localhost:1", Html5LocalHttpServer.readLineBounded(r, 8192))
        assertEquals("", Html5LocalHttpServer.readLineBounded(r, 8192))
        assertNull("exhausted stream reads as end of request", Html5LocalHttpServer.readLineBounded(r, 8192))
    }

    @Test
    fun readLineBounded_returnsTrailingContentWithoutNewline() {
        assertEquals("GET /x HTTP/1.1", Html5LocalHttpServer.readLineBounded(reader("GET /x HTTP/1.1"), 8192))
    }

    // a local peer that opens a socket and never sends a newline must not grow the heap one char
    // at a time until the read timeout fires.
    @Test(expected = java.io.IOException::class)
    fun readLineBounded_throwsPastTheCap() {
        Html5LocalHttpServer.readLineBounded(reader("A".repeat(64)), 16)
    }

    @Test
    fun readLineBounded_acceptsALineExactlyAtTheCap() {
        val line = "A".repeat(16)
        assertEquals(line, Html5LocalHttpServer.readLineBounded(reader(line + "\n"), 16))
    }
