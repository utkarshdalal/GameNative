...
class ProfileSchemaTest {
    ...
    fun minimal_json_parses_with_defaults() {
        ...
        // input + saves have non-null host-level defaults, so pack JSONs omit them when they
        // match (pointer-with-tap-detection input, leveldb-origin-rewrite saves with pcOrigin "file://").
    ...
    // runtime, so pinning "fsbridge" here would strand leveldb-saving builds.
    @Test
    ...
    // loads each SHIPPED pack JSON from assets and runs the production resolveShimUrls, catching a
    // wrong/missing packShimPlacement in the JSON AND a resolver regression -- the
    // WebViewScreenShimResolutionTest unit tests only use hand-built profiles. pack shim id + url
    // are derived by convention (pack:foo -> pack-foo -> /_shims/packs/foo.js).
    ...
    // a malformed pack JSON must swallow + log + return null -- Html5OptInService maps that to
    // Result.PackLoadFailure, so a throw here is a crash there.
    @Test
