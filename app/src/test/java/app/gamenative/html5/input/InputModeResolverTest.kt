...
class InputModeResolverTest {
    ...
    @Test fun container_whitespace_treated_as_blank() {
        // isNotBlank() means " " falls through -- defensive, in case whitespace is ever persisted
        assertEquals(
            "native-controller",
            resolveInputMode(container("   "), profile("native-controller")),
        )
    ...
    @Test fun schema_default_constant_matches_InputSpec_default() {
        // if InputSpec.mode's default changes, SCHEMA_DEFAULT_INPUT_MODE must follow or it
        // diverges from the default seen by profile parsing.
