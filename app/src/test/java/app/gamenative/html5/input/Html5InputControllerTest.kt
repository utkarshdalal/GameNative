...
class Html5InputControllerTest {
    ...
    fun onKeyEvent_non_back_key_returns_false_so_WebView_sees_native_KeyEvent() {
        ...
        // relaxed so onKeyEvent's source/repeatCount reads get default 0 -- the keyCode stub
        // still wins per MockK explicit-over-relaxed.
    ...
    fun onKeyEvent_back_key_after_ButtonB_is_consumed_as_controller_B() {
        // Android handhelds emit BUTTON_B + KEYCODE_BACK together (~10ms apart) for one physical
        // controller-B press. co-occurrence heuristic: BACK within CONTROLLER_B_WINDOW_MS of a
        // BUTTON_B is the controller, not hardware-back -> consume.
        ...
        // first call returns t=100 (when BUTTON_B fires), later calls t=110 (10ms later -- same
        // physical press)
    ...
    fun null_profile_during_consume_leaves_bridge_at_default_without_crash() {
        ...
        // null profile: handler still recognizes the event (mocked) but updateBridgeFromProfile
        // early-returns, bridge untouched.
        ...
        // onKeyEvent always returns false (native WebView pass-through).
        assertEquals(false, result)
    ...
    // when MULTIPLE axes resolve to the SAME logical key on the active profile (e.g. AXIS_X+ and
    // DPAD_RIGHT both bound to KEY_D), processing one MotionEvent must NOT serialize keydown->keyup
    // on the same tick: an inactive axis calling onAxisValue(KEY_D, 0f) right after the active one
    // called onAxisValue(KEY_D, 1f) trips the synthesizer's keyup transition. aggregation in
    ...
    fun synthesizeAxisKeys_two_axes_same_key_only_active_axis_wins() {
        // AXIS_X+ -> KEY_D AND DPAD_RIGHT -> KEY_D.
        ...
        // would otherwise let the catch-all overwrite the specific stubs.
        ...
        // AXIS_X=1.0 (active right), AXIS_HAT_X=0.0 (dpad neutral), all other axes neutral.
    ...
    // HAT_Y=-1.0 (d-pad UP) must produce one ArrowUp keydown when the profile binds DPAD_UP
    // keycode to KEY_UP directly (RMMV/Electron shape). pack:c3 profiles keep GAMEPAD_DPAD_UP and
    // would produce zero keydowns here -- intended, since gamepad goes via the virtual gamepad
    // bridge, not synth.
    @Test
    fun synthesizeAxisKeys_hat_y_negative_dispatches_arrow_up() {
        ...
        // RMMV-pack profile: DPAD keycodes bound directly to KEY_UP/DOWN via gamepadKeySynthesisMap
        // applied at default-profile-population time.
        every { controller.getControllerBinding(any<Int>()) } answers {
            when (firstArg<Int>()) {
    ...
    // at profile-creation time using `gamepadKeySynthesisMap`.
    @Test
    fun dispatchBinding_gamepad_does_not_synthesize_keyboard_events() {
        ...
        val drained = bridge.drainQueue()
        // GAMEPAD_* dispatches go to handler.applyBinding, not the synthesizer's __gnInputBridge queue.
    ...
    // dispatchBinding for GAMEPAD_BUTTON_A routes through handler.applyBinding
    ...
    // thumb-direction binding with offset=0 defaults to full deflection sign.
    ...
    // GAMEPAD_LEFT_THUMB_RIGHT digital tap -> +1f deflection (matches +ve sign).
    @Test
    ...
    // L2/R2 with offset=0 defaults to full press (1f) when isDown=true, matching the physical
    // KeyEvent path's default for digital trigger presses.
    @Test
    ...
    // analog overlay (joystick) passes a real magnitude -> preserves it instead
    ...
    fun dispatchBinding_thumbDirection_analogOffset_passesThrough() {
        ...
        // 50% deflection -- non-zero offset, must NOT be defaulted.
    ...
    // onKeyEvent for a GAMEPAD_*-resolved binding must NOT also route through dispatchBinding
    // (handler.onKeyEvent already wrote profile.gamepadState -- a second routing would duplicate
    // the write).
    @Test
    fun onKeyEvent_gamepad_resolvedBinding_does_not_route_through_dispatchBinding() {
        ...
        // handler.onKeyEvent ALWAYS runs. handler.applyBinding must NOT be called (that would be
        // the duplicate write from dispatchBinding's gamepad branch). the gate is the
        // !effective.isGamepad check in onKeyEvent.
    ...
    // profile binds DPAD keycodes to KEY_LEFT/RIGHT directly. HAT_X=1.0 -> KEYCODE_DPAD_RIGHT ->
    // KEY_RIGHT -> ArrowRight keydown.
    @Test
