...
// IO-level contract: failure classification, blob copying, a real-save smoke test. key-level rewrite
// coverage lives in LevelDbRewriterIdbTest + LevelDbRewriterLsTest.
class LevelDbRewriterTest {
    ...
    fun rewrite_permissionDeniedOnDst_throwsPermissionDenied() {
        // chmod semantics differ on windows test runs
        ...
        // at least one key so the rewriter gets past open-src
        ...
        try {
            ...
            try {
                ...
                // some JVMs wrap EACCES in a generic IOException; still a SaveSyncFailure, which is the contract
                assertTrue("other failure accepted when JVM masks EACCES", true)
            } catch (e: SaveSyncFailure.PathMissing) {
                // on some POSIX JVMs dst.mkdirs() fails silently and the next File op throws FileNotFoundException
    ...
    // skipped unless GAMENATIVE_HTML5_SAVE_FIXTURE_ROOT provides the fixture
    @Test
    fun rewrite_realSolCestoIdbFixture_roundtripsWithIdb1() {
        ...
        // snapshot first: iq80 mutates the dir on recovery
        ...
        // the ASCII prefix is the same chars as the IDB filename form
        ...
        openDb(dst, readOnly = true, useIdb1 = true).use { db ->
            db.iterator().use { iter ->
                ...
                while (iter.hasNext()) {
                    ...
                    e.value
                    count++
                    if (count >= 20) break // real IDB can be large
                }
            }
        }
    ...
    fun copyLiveBlobs_copiesOnlyLiveRefs_skipsOrphans() {
        ...
        // orphan: no live ref (chromium blob-journal GC lag)
    ...
    fun copyLiveBlobs_matchesHexFilename() {
        // chromium spells blob numbers >= 10 in hex
    ...
    @Test
    fun copyLiveBlobs_matchesHexDbIdDirectory() {
        // the db_id DIRECTORY is hex too: a decimal-only lookup misses every blob under db_id >= 10.
        // below 10 the spellings are identical, which hides it on typical saves.
        val src = File(tmpRoot, "blob-src-hexdb").also { it.mkdirs() }
        File(src, "a/00").also { it.mkdirs() }
        File(src, "a/00/1").apply { writeBytes(byteArrayOf(0xAB.toByte())) } // db_id 10 spelled hex

        val dst = File(tmpRoot, "blob-dst-hexdb")
        LevelDbRewriter.copyLiveBlobs(src, dst, setOf(10 to 1))

        assertTrue("blob under a hex db_id dir must be found", File(dst, "a/00/1").isFile)
    }

    @Test
    fun copyLiveBlobs_decimalDbIdDirectoryStillWins() {
        // hex lookup is additive, not a switch
        val src = File(tmpRoot, "blob-src-decdb").also { it.mkdirs() }
        File(src, "10/00").also { it.mkdirs() }
        File(src, "10/00/1").apply { writeBytes(byteArrayOf(0xCD.toByte())) }

        val dst = File(tmpRoot, "blob-dst-decdb")
        LevelDbRewriter.copyLiveBlobs(src, dst, setOf(10 to 1))

        assertTrue(File(dst, "10/00/1").isFile)
    }
    ...
    fun copyLiveBlobs_emptyLiveRefs_wipesDstNoCopy() {
        // no IDB records left (all DBs deleted): stale wine-side blobs from prior sessions must not linger
