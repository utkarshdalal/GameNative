...
// the captured fixture (a real game's thumbnail record) comes from Ff1102FixtureExtractor.
class LevelDbRewriterSnappyValueTest {
    ...
    fun decompressesCapturedWaywardRecord() {
        ...
        // V8 SSV v15 right after the wrapper: `ff 0f 6f` (version 15 + beginObject)
        assertEquals(0xFF.toByte(), result[16])
        assertEquals(0x0F.toByte(), result[17])
        assertEquals(0x6F.toByte(), result[18])

        // snappy declared uncompressed size 76547 (varint `83 d6 04`) + 1 leading varint byte
        assertEquals(76548, result.size)
    }

    // post-maybeInlineBlobValue shape for a ff 11 01 (blob-wrapped) record: main value contributes
    // <leading_varint>, sidecar bytes start with ff 11 02. the sidecar is synthesized via Snappy.compress
    // rather than shipping a ~0.5MB captured blob; the captured-record test stays the real-data anchor.
    @Test
    fun decompressesInlinedBlobSidecar() {
        // large + varied so rawUncompress exercises a real multi-KB output buffer, not a single block
    ...
    fun blobWrappedValuePassesThrough() {
        // ff 11 01 (kReplaceWithBlob) belongs to maybeInlineBlobValue
