...
// fsbridge titles must short-circuit BEFORE SaveDirectoryResolver.resolve so stripped saves blocks
// don't throw PathMissing.
...
class Html5SaveSyncServiceFsBridgeTest {
    ...
    fun setUp() {
        ...
        // Container has static clinit side effects but is directly constructable under robolectric
    ...
    fun mirrorOnFlip_fsbridgeProfile_skipsResolverAndNoSnackbar() {
        ...
        // fsbridge returning null from resolveSetup is a legitimate no-op, not an error to surface
        verify(exactly = 0) { SnackbarManager.show(any()) }
    }

    @Test
    fun syncInbound_fsbridgeProfileWithStrippedSavesBlock_skipsResolver() {
        // saves=null maps to FsBridge; without the short-circuit SaveDirectoryResolver.resolve would throw
