...
// debug tool: walks the V8 ScriptValueSerializer (SSV) tag stream inside each IDB record value, to find
// which tag/sub-encoding an older WebView (V8 ~10.7) silently fails on in saves authored by newer Chromium.
// usage: un-@Ignore, then ./gradlew :app:testDebugUnitTest --tests 'app.gamenative.html5.savesync.SsvTagWalker'
...
class SsvTagWalker {
        ...
        Iq80DBFactory.factory.open(File(ldbPath), opts).use { db ->
            db.iterator().use { it ->
                ...
                while (it.hasNext()) {
                    ...
                    val k = entry.key
                    val v = entry.value
                    // walk every record, metadata included, and flag walked vs aborted
    ...
    private fun walkOne(index: Int, key: ByteArray, value: ByteArray, out: StringBuilder) {
        ...
        // outer envelope: optional ff <ver> fe <12 null bytes> (newer Chromium)
        if (r.peek() == 0xFF) {
            r.read()
            val outerVer = r.read()
    ...
    private fun walkValue(r: Reader, out: StringBuilder, depth: IntArray, indent: Int): Boolean {
        ...
        when (tag) {
            ...
            0x56 -> { // kArrayBufferView
                ...
                // peek to tell them apart: if the next byte looks like a tag, flags was omitted.
                val markerPos = r.pos
    ...
    // kTwoByteString bytes are little-endian UTF-16 code units
