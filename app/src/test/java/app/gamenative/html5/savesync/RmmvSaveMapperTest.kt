...
// chromium localStorage LevelDB <-> .rpgsave passthrough. synthetic DBs cover the framing + key mapping
// without on-device fixtures.
class RmmvSaveMapperTest {
    ...
    // inbound writes the player's .rpgsave files at launch. writing in place lets a kill mid-write truncate the
    // existing save; staging to a sibling and renaming means a failed staging leaves the previous save whole.
    @Test
    fun writeLocalStorageToFiles_stagingFailureLeavesPreviousSaveIntact() {
        val src = File(tmpRoot, "ls-src-atomic").also { it.mkdirs() }
        val saveDir = File(tmpRoot, "saves-atomic").also { it.mkdirs() }
        val previous = "PREVIOUS-SAVE".toByteArray(Charsets.US_ASCII)
        File(saveDir, "file1.rpgsave").writeBytes(previous)
        File(saveDir, "file1.rpgsave.gntmp").mkdirs()

        openDb(src).use { db -> db.put(makeKey("RPG File1"), frameValue(ByteArray(64) { 9 })) }

        runCatching { RmmvSaveMapper.writeLocalStorageToFiles(src, origin, saveDir) }

        assertArrayEquals(previous, File(saveDir, "file1.rpgsave").readBytes())
    }

    @Test
    fun writeLocalStorageToFiles_leavesNoStagingFilesBehind() {
        val src = File(tmpRoot, "ls-src-clean").also { it.mkdirs() }
        val saveDir = File(tmpRoot, "saves-clean")
        openDb(src).use { db -> db.put(makeKey("RPG File1"), frameValue(ByteArray(32) { 7 })) }

        RmmvSaveMapper.writeLocalStorageToFiles(src, origin, saveDir)

        assertEquals(listOf("file1.rpgsave"), saveDir.list()!!.sorted())
    }
    ...
    fun writeLocalStorageToFiles_ignoresNonRpgKeys() {
        ...
        openDb(src).use { db ->
            ...
            // chromium's META: key must not match the filter
