...
// a single off-by-one in varint encoding or a wrong CRC mask silently corrupts the synthesized MANIFEST
// and iq80 fails to open it, so the wire formats are locked here.
//
// references:
...
class LevelDbRewriterEncodingTest {
    ...
    @Test fun varint_128_isTwoBytes() {
        ...
        // low 7 bits = 0 with continuation, then 1
    ...
    @Test fun varint_184_isTwoBytes_realFileNumberFromProductionRepro() {
        // low 7 bits = 0x38, with continuation = 0xB8; then 0x01
        val out = ByteArrayOutputStream()
    ...
    @Test fun versionEdit_encodesComparator_logNumber_nextFileNumber_lastSequence_newFiles() {
        ...
        // varint(1_000_000_000) is 5 bytes; exact encoding is covered by the varint tests
        pos += 5
        ...
        assertEquals(0x01.toByte(), body[pos]); pos++
        // file size 570976: 3-byte varint, not checked byte-for-byte
        pos += 3
    ...
    @Test fun versionEdit_multipleNewFiles_writeInOrder() {
        ...
        // a floor, not an exact count: 7 could appear inside payloads, though these values are chosen so it doesn't
    ...
    @Test fun versionEdit_idbComparatorName_isUsedVerbatim() {
        // chromium IDB and iq80's CustomUserComparator(Idb1Comparator()) both require the literal "idb_cmp1"
        val body = LeveldbManifestSynthesizer.encodeVersionEdit(
